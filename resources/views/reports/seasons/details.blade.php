...
<div class="container-fluid">
    <div class="text-right pb-2">
        <a href="{{ route('report.seasons') }}"><button class="btn btn-sm btn-primary">Back</button></a>
    </div>
    <!-- DataTales Example -->
    ...
    <div class="card-header py-3">
        <div class="row">
            <div class="col-12 col-md-2">
                <img style="width:100%" src="{{ asset('storage/series/seasons/'.$season->cover) }}">
            </div>
            <div class="col-6 col-md-7">
                <h4 class="font-weight-bold text-uppercase text-primary">{{ $season->series->title }}</h4>
                <h5 class="font-weight-bold text-uppercase text-primary">{{ $season->title }}</h5>
                <h6 class="font-weight-bold text-uppercase mb-0">Discription</h6>
                {!! $season->description !!}

            </div>
            <div class="col-6 col-md-3">
                <h6 class="font-weight-bold text-uppercase mb-0">Season No</h6>
               {{ $season->season_no }}
                <h6 class="font-weight-bold text-uppercase">Lenguage</h6>
                {{ $season->series->language }}
                <h6 class="font-weight-bold text-uppercase mb-0">Release Date</h6>
                {{  Carbon\Carbon::parse($season->release_date)->format('d-m-Y')  }}
    ...
    <div class="d-flex justify-content-right">
      <div class="pb-3" style="width:40%;">
          ...
          <select class="custom-select" name="filter" id="filter" season-id="{{ $season->id }}">
                @php $i = 1; @endphp
                @for($i== 1; $i<= 12; $i++)
      ...
      <div class="table-responsive">
        <table class="table table-bordered datatable" width="100%" cellspacing="0" id="detailsTable" style="width:100%;">
          <thead>
            <tr>
            <th>#</th>
    ...
    $(document).ready(function() {
       var table =  $('#detailsTable').DataTable({
        ...
        buttons : [ 
            {
                extend: 'print',
                title: '<h4 style="text-align:center">Season Report</h4> <h5 class="text-center">Season Title - {{ $season->title  }}</h5>',
        ...
        "ajax": {
            url: "{{ route('report.season.montly') }}",
            type: 'POST',
            "dataSrc": "payments",
            data: function(d) {
                d.season_id = $('#filter').attr('season-id');
                d.month = $('#filter').val();
            },
